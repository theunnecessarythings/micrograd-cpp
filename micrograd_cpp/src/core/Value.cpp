...
#include <algorithm>
#include <stdexcept>
#include <set> // Required for std::set in constructors
...
// --- Activation functions & other math ---
...
std::shared_ptr<Value> Value::tanh() {
    ...
    float x = self->data;
    float t = std::tanh(x);
    auto out = std::make_shared<Value>(t, "tanh", std::set<std::shared_ptr<Value>>{self});
    out->_backward = [self, out, t]() {
        self->grad += (1.0f - t * t) * out->grad;
    };
    return out;
}
...
std::shared_ptr<Value> Value::log() {
    ...
    out->_backward = [self, out]() {
        float grad_val = (self->data == 0.0f) ? 0.0f : (1.0f / self->data);
